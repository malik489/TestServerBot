...
bot = TeleBot(token="7113305887:AAF2ypc5_VcgbIGFPh8Kz2VHFbaBJXHxnFw") # توكن بوت الرشق الاساسي هنا
if not db.get('accounts'):
    db.set('accounts', [])
    pass
db.delete("force")
admin = 1284148106 #الادمن
db.set('admins', [admin, 5803236060])
...
def start_message(message):
    ...
    if not db.get(f'user_{user_id}'):
        ...
        btn2 = btn('خزمەتگوزاریەکان🛍', callback_data='ps')
        ...
        btn6 = btn('قناة البوت 🩵', url='https://t.me/mallik489')
...
def c_rs(call):
    ...
    if data == 'ps':
        keys = mk(row_width=2)
        btn2 = btn('بەشی فری',callback_data='free')
    ...
    if data == 'free':
        ...
        for temp in a:
            ...
            db.delete(f'{a}_{user_id}_proccess')
        keys = mk(row_width=2)
        btn2 = btn('دەنگدان',callback_data='votes')
        ...
        btn7 = btn('زیادکردنی ڤیو',callback_data='view')
...
{y}'''
    ...
    if data == 'votes':
        ...
        x = bot.edit_message_text(text=' ژمارەی دەنگەکان',reply_markup=bk,chat_id=cid,message_id=mid)
        type = 'votes'
        bot.register_next_step_handler(x, get_amount, type)
    if data == 'buy':
        keys = mk(row_width=2)
...
✯𝑫𝒆𝒗 »» '''
    ...
    if data == 'dump_votes':
        ...
        if prem is True:
            ...
            x = bot.edit_message_text(text='•لینکی دەنگ دانەکە بنێرە',reply_markup=bk,chat_id=cid,message_id=mid)
...
{y}
    ...
    if data == 'view':
        ...
        x = bot.edit_message_text(text='ژمارەی ڤیوەکان',reply_markup=bk,chat_id=cid,message_id=mid)
        type = 'view'
        bot.register_next_step_handler(x, get_amount, type)
    if data == 'poll':
    ...
    if type == 'votes':
        ...
        if message.text:
            ...
            if amount < 1:
                ...
                return
            if amount > 200:
                bot.reply_to(message,'رجاء ارسل عدد اكبر من  200 ',reply_markup=bk)
                return
            ...
            if len(load_) < amount:
                ...
                return
            x = bot.reply_to(message,f'• ژمارەی دیاری کراوی دەنگ {amount} \n• کاتی نێوان دەنگەکان دیاری بکە (بە چرکە) \n\n•بۆ خیراترین دەنگ 0 دابگرا و ژمارەی چرکە لە 200 زیاتر نەبێت ')
